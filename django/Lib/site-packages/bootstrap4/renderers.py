...
class FieldRenderer(BaseRenderer):
    ...
    def list_to_class(self, html, klass):
        ...
        if DJANGO_VERSION >= 4:
            ...
            for inner_div in enclosing_div.find_all("div"):
                inner_div.attrs["class"] = inner_div.attrs.get("class", []) + [
                    self.form_check_class
                ]
        else:
            mapping = [
                ...
                (
                    "<li",
                    '<div class="{form_check_class}"'.format(
                        form_check_class=self.form_check_class
                    ),
                ),
                ("</li>", "</div>"),
        ...
        if enclosing_div:
            for label in enclosing_div.find_all("label"):
                label.attrs["class"] = label.attrs.get("class", []) + [
                    "form-check-label"
                ]
                try:
                    label.input.attrs["class"] = label.input.attrs.get("class", []) + [
                        "form-check-input"
                    ]
    ...
    def make_input_group_addon(self, inner_class, outer_class, content):
        if not content:
            return ""
        if inner_class:
            content = '<span class="{inner_class}">{content}</span>'.format(
                inner_class=inner_class, content=content
            )
        return '<div class="{outer_class}">{content}</div>'.format(
            outer_class=outer_class, content=content
        )
    ...
    def make_input_group(self, html):
        if self.is_input_group:
            ...
            html = self.append_errors(
                "{before}{html}{after}".format(before=before, html=html, after=after)
            )
    ...
    def wrap_field(self, html):
        ...
        if field_class:
            html = '<div class="{field_class}">{html}</div>'.format(
                field_class=field_class, html=html
            )
        return html
