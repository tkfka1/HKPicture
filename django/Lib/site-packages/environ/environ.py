...
class Env(object):
    ...
    def db_url_config(cls, url, engine=None):
        ...
        if url.scheme == "sqlite":
            ...
            if url.netloc:
                warnings.warn(
                    "SQLite URL contains host component %r, it will be ignored"
                    % url.netloc,
                    stacklevel=3,
                )
        if url.scheme == "ldap":
            path = "{scheme}://{hostname}".format(
                scheme=url.scheme, hostname=url.hostname
            )
            if url.port:
...
class Path(object):
    ...
    def __add__(self, other):
        return Path(
            self.__root__, other if not isinstance(other, Path) else other.__root__
        )
