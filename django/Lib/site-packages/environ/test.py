...
from .compat import json, DJANGO_POSTGRES, ImproperlyConfigured, REDIS_DRIVER, quote
...
class EnvTests(BaseTests):
    ...
    def test_str_list_with_spaces(self):
        self.assertTypeAndValue(
            list, [" foo", "  bar"], self.env("STR_LIST_WITH_SPACES", cast=[str])
        )
        self.assertTypeAndValue(
            list, [" foo", "  bar"], self.env.list("STR_LIST_WITH_SPACES")
        )
    ...
    def test_dict_parsing(self):
        ...
        self.assertEqual(
            {"a": ["1", "2", "3"]}, self.env.parse_value("a=1,2,3", dict(value=[str]))
        )
        self.assertEqual(
...
class EmailTests(unittest.TestCase):
    def test_smtp_parsing(self):
        ...
        self.assertEqual(
            url["EMAIL_BACKEND"], "django.core.mail.backends.smtp.EmailBackend"
        )
...
class PathTests(unittest.TestCase):
    ...
    def test_required_path(self):
        ...
        self.assertRaises(
            ImproperlyConfigured, Path(__file__), "not_existing_path", required=True
        )
    ...
    def test_comparison(self):
        ...
        self.assertEqual(
            Path("/home/foo/").rfind("/"), str(Path("/home/foo")).rfind("/")
        )
        self.assertEqual(
            Path("/home/foo/").find("/home"), str(Path("/home/foo/")).find("/home")
        )
